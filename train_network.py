...
def train(args, tuning_mode=False):
    ...
    cache_latents, use_dreambooth_method, use_user_config, tokenizer, train_dataset_group, train_collater, val_collater = ready_dataset(args, current_epoch, current_step)
    ...
    # モデルを読み込む
    text_encoder, vae, unet = load_models(args, cache_latents, train_dataset_group, accelerator, weight_dtype)
    ...
    net_kwargs, network, train_unet, train_text_encoder = prepare_networks(args, text_encoder, vae, unet, network_module)  # may have no effect
    ...
    # epoch数を計算する
    num_train_epochs = get_epoch_count(args, train_dataloader)
    ...
    if is_main_process:
        log_startup(args, train_dataset_group, train_dataloader, num_train_epochs)

    # TODO refactor metadata creation and move to util
    metadata = create_metadata(args, session_id, training_started_at, use_dreambooth_method, use_user_config, train_dataset_group, optimizer_name, optimizer_args, train_dataloader, num_train_epochs, total_batch_size)
    ...
    # model name and hash
    get_model_info(args, metadata)

    if args.vae is not None:
        get_vae_info(args, metadata)
    ...
    for epoch in range(num_train_epochs):
        ...
        train_epoch(args, tokenizer, current_epoch, current_step, accelerator, unwrap_model, weight_dtype, text_encoder, vae, unet, network, train_text_encoder, optimizer, train_dataloader, val_dataloader, lr_scheduler, metadata, progress_bar, global_step, noise_scheduler, loss_list, loss_total, on_step_start, save_model, remove_model, epoch)
        validate_epoch(args, tokenizer, current_epoch, current_step, accelerator, unwrap_model, weight_dtype, text_encoder, vae, unet, network, train_text_encoder, optimizer, train_dataloader, val_dataloader, lr_scheduler, metadata, progress_bar, global_step, noise_scheduler, loss_list, loss_total, on_step_start, save_model, remove_model, epoch)
        # 指定エポックごとにモデルを保存
...
def log_startup(args, train_dataset_group, train_dataloader, num_train_epochs):
...
def load_models(args, cache_latents, train_dataset_group, accelerator, weight_dtype):
...
def prepare_configuration(args):
    ...
    if use_user_config:
        ...
        user_config = choose_training_method(args, use_dreambooth_method)

    train_dataset_group = generate_dataset_for_training(args, tokenizer, blueprint_generator, user_config)
    return cache_latents,use_dreambooth_method,use_user_config,tokenizer,train_dataset_group
def generate_dataset_for_training(args, tokenizer, blueprint_generator, user_config):
    blueprint = blueprint_generator.generate(user_config, args, tokenizer=tokenizer)
    train_dataset_group = config_util.generate_dataset_group_by_blueprint(blueprint.dataset_group)
    return train_dataset_group

def get_default_noise_scheduler():
...
def generate_minimum_metadata(metadata):
...
def get_vae_info(args, metadata):
    vae_name = args.vae
...
def get_model_info(args, metadata):
...
def create_metadata(args, session_id, training_started_at, use_dreambooth_method, use_user_config, train_dataset_group, optimizer_name, optimizer_args, train_dataloader, num_train_epochs, total_batch_size):
    metadata = {
...
def prepare_networks(args, text_encoder, vae, unet, network_module):
    net_kwargs = {}
    ...
    return net_kwargs,network,train_unet,train_text_encoder

def cache_latents_on_device(args, dataset_group, accelerator, weight_dtype, vae):
    vae.to(accelerator.device, dtype=weight_dtype)
    vae.requires_grad_(False)
    vae.eval()
    with torch.no_grad():
        dataset_group.cache_latents(vae, args.vae_batch_size, args.cache_latents_to_disk, accelerator.is_main_process)
    vae.to("cpu")
...
def choose_training_method(args, use_dreambooth_method):
    if use_dreambooth_method:
...
def do_step(args, tokenizer, current_step, accelerator, weight_dtype, text_encoder, vae, unet, network, train_text_encoder, optimizer, lr_scheduler, global_step, noise_scheduler, on_step_start, batch):
    ...
    with accelerator.accumulate(network):
        ...
        loss = compute_loss_from_latents(args, tokenizer, accelerator, weight_dtype, text_encoder, vae, unet, train_text_encoder, noise_scheduler, batch)
...
def compute_loss_from_latents(args, tokenizer, accelerator, weight_dtype, text_encoder, vae, unet, train_text_encoder, noise_scheduler, batch):
    ...
    encoder_hidden_states = get_hidden_states(args, tokenizer, accelerator, weight_dtype, text_encoder, train_text_encoder, batch)
...
def get_hidden_states(args, tokenizer, accelerator, weight_dtype, text_encoder, train_text_encoder, batch):
